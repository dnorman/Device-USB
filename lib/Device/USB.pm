...
int usbwrap_find_busses()
{
    return usb_find_busses();
}

int usbwrap_find_devices()
{
    return usb_find_devices();
}

void *usbwrap_get_busses()
{
    return usb_get_busses();
}

void *usbwrap_open(void *dev)
{
    return usb_open( (struct usb_device*)dev );
}

int usbwrap_close(void *dev)
{
    return usb_close((usb_dev_handle *)dev);
}

int usbwrap_set_configuration(void *dev, int configuration)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_set_configuration( %d )\n", configuration );
    }
    return usb_set_configuration((usb_dev_handle *)dev, configuration);
}

int usbwrap_set_altinterface(void *dev, int alternate)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_set_altinterface( %d )\n", alternate );
    }
    return usb_set_altinterface((usb_dev_handle *)dev, alternate);
}

int usbwrap_clear_halt(void *dev, unsigned int ep)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_clear_halt( %d )\n", ep );
    }
    return usb_clear_halt((usb_dev_handle *)dev, ep);
}

int usbwrap_reset(void *dev)
{
    return usb_reset((usb_dev_handle *)dev);
}

int usbwrap_get_driver_np(void *dev, int interface, char *name, unsigned int namelen)
{
    int ret = 0;
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_get_driver_np( %d )\n", interface );
    }
...
int usbwrap_detach_kernel_driver_np(void *dev, int interface)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_detach_kernel_driver_np( %d )\n", interface );
    }
...
int usbwrap_claim_interface(void *dev, int interface)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_claim_interface( %d )\n", interface );
    }
    return usb_claim_interface((usb_dev_handle *)dev, interface);
}

int usbwrap_release_interface(void *dev, int interface)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_release_interface( %d )\n", interface );
    }
    return usb_release_interface((usb_dev_handle *)dev, interface);
}

void usbwrap_control_msg(void *dev, int requesttype, int request, int value, int index, char *bytes, int size, int timeout)
{
    int i = 0;
    int retval = 0;
    ...
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_control_msg( %#04x, %#04x, %#04x, %#04x, %p, %d, %d )\n",
...
int usbwrap_get_string(void *dev, int index, int langid, char *buf, size_t buflen)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_get_string( %d, %d, %p, %u )\n",
...
int usbwrap_get_string_simple(void *dev, int index, char *buf, size_t buflen)
{
    if(DeviceUSBDebugLevel())
    {
        printf( "usbwrap_get_string_simple( %d, %p, %u )\n",
            index, buf, buflen
        );
    }
...
int usbwrap_get_descriptor(void *dev, unsigned char type, unsigned char index, void *buf, int size)
{
    return usb_get_descriptor((usb_dev_handle *)dev, type, index, buf, size);
}

int usbwrap_get_descriptor_by_endpoint(void *dev, int ep, unsigned char type, unsigned char index, void *buf, int size)
{
    return usb_get_descriptor_by_endpoint((usb_dev_handle *)dev, ep, type, index, buf, size);
}

int usbwrap_bulk_write(void *dev, int ep, char *bytes, int size, int timeout)
{
    return usb_bulk_write((usb_dev_handle *)dev, ep, bytes, size, timeout);
}

int usbwrap_bulk_read(void *dev, int ep, char *bytes, int size, int timeout)
{
    return usb_bulk_read((usb_dev_handle *)dev, ep, bytes, size, timeout);
}

int usbwrap_interrupt_write(void *dev, int ep, char *bytes, int size, int timeout)
{
    return usb_interrupt_write((usb_dev_handle *)dev, ep, bytes, size, timeout);
}

int usbwrap_interrupt_read(void *dev, int ep, char *bytes, int size, int timeout)
{
